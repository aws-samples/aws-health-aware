...
def get_sts_token():
    ...
    if "arn:aws:iam::" in assumeRoleArn:
        ...
        print("Running in management account demployment mode")
    
    return health_client

def main(event, context):
    print("THANK YOU FOR CHOOSING AWS HEALTH AWARE!")
    health_client = get_sts_token()
    ...
    if org_status == "No":
        ...
        aws_events = describe_events(health_client)
        ...
        aws_events = describe_org_events(health_client)
        aws_events = json.dumps(aws_events, default=myconverter)
        aws_events = json.loads(aws_events)
        print('Event(s) Received: ', json.dumps(aws_events))
        if len(aws_events) > 0:
